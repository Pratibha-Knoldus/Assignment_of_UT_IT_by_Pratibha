...
class IT_Test_UserImpl extends AsyncFunSuite{
  ...
  val user1 : User = User("Pratibha","Yadav",21,50000,"Engineer","Knoldus","pratibhayadav@knoldus.com")
  test("Correct user, Company name & EmailId"){
    assert(userImpl.createUser(user1).contains(user1.emailId))
  }

  val user2 : User = User("Suresh","Gupta",21,50000,"Engineer","Knoldus","pratibhayadav@knoldus.com")
  test("Wrong user name & Correct Company name & EmailId"){
    println()
    assert(!userImpl.createUser(user2).contains(user2.emailId))
  }

  val user3 : User = User("Pratibha","Yadav",21,50000,"Engineer","Google","pratibhayadav@knoldus.com")
  test("Correct user,EmailId & Wrong Company name"){
    println()
    assert(!userImpl.createUser(user3).contains(user3.emailId))
  }

  val user4 : User = User("Pratibha","Yadav",21,50000,"Engineer","Knoldus","pratibhayadavknoldus.com")
  test("Correct user, Company name & Wrong EmailId"){
    println()
    assert(!userImpl.createUser(user4).contains(user4.emailId))
  }

  val user5 : User = User("Suresh","Gupta",21,50000,"Engineer","Google","pratibhayadavknoldus.com")
